...
function toInterfaceCombinator(schema) {
    ...
    return t.interfaceCombinator(Object.keys(schema.properties).map(function (key) { return t.property(key, to(schema.properties[key]), !required.hasOwnProperty(key)); }));
}
function to(schema) {
    switch (schema.type) {
        case 'string':
